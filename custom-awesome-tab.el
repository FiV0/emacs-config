  ...
  (list
   (let ((name (buffer-name)))
     (cond ((or (and (>= (length name) 6) (string-equal "*slime" (substring name 0 6)))
                ...
                (and (>= (length name) 5) (string-equal "*ielm" (substring name 0 5)))
                (and (>= (length name) 7) (string-equal "*Python" (substring name 0 7))))
            "repl")
           ((and (>= (length name) 1) (string-equal "*" (substring name 0 1)))
            "emacs")
           ((eq major-mode 'dired-mode)
            "dired")
           ((and (>= (length name) 5) (string-equal "magit" (substring name 0 5)))
            "magit")
           (t "user")))))
